...
// ============================================================
// PDX-51 — Pantalla Registro
// PDX-53 — OAuth Google + GitHub con Supabase Auth
// PDX-54 — Validación en tiempo real, isValid, contador, FormError
// ============================================================
...
const registerSchema = z
  .object({
    username: z
      .string()
      .min(1, 'El nombre de entrenador es requerido')
...
function PasswordStrength({ password }: { password: string }) {
  ...
  const label = ['', 'Débil', 'Regular', 'Buena', 'Fuerte'][score]
        ...
        {[1, 2, 3, 4].map((bar) => (
          <div
            ...
            className={`h-1 flex-1 rounded-full transition-colors duration-300 ${
              bar <= score ? barColor : 'bg-zinc-200 dark:bg-zinc-800'
            }`}
          />
        ))}
      </div>
      <p className={`font-mono text-xs ${textColor}`}>{label}</p>
    </div>
  )
}
    ...
    >
      ...
      <motion.div className="flex flex-col gap-2" variants={itemVariants}>
        ...
        <p className="font-sans text-sm text-zinc-500 dark:text-zinc-400">
          Empezá tu aventura como entrenador
        </p>
      </motion.div>
      ...
      >
        ...
        <div className="flex flex-col gap-1">
          <Input
            ...
            isValid={
              touched.username && !errors.username && usernameValue.length >= 3
            }
            {...register('username')}
          ...
          {usernameValue.length > 0 && (
            ...
            >
              {usernameValue.length} / 30 caracteres
            </p>
          )}
        </div>
        ...
        <Input
          ...
          isValid={
            touched.confirmPassword && !errors.confirmPassword && !!confirmValue
          }
        ...
        <div className="flex flex-col gap-1.5">
          <div className="flex items-start gap-2.5">
            ...
            <label
              htmlFor="terms"
              className="font-sans text-xs text-zinc-500 dark:text-zinc-400"
            >
